...
function Header({sendDataToHome}){
 ...
 const {Token,submitlogout,currentUser,userID}=useContext(OrderContext);
...
withCredentials:true,
 headers:{ "Authorization":` Token ${Token}`}
  ...
  try{
    ...
    console.log("get username from getDATA ",profiles.data.username);
    console.log("get ID from getDATA ",profiles.data.id);
    

    setusername(profiles.data.username);
    setAddress(profiles.data.Address);
   ...
   <Col> <h3 style={{marginLeft:"10px",marginTop:"10px"}}>{username}</h3></Col>
     ...
     <Button variant="outline-success" style={{bottom:"10px",position:"absolute",right:"5px"}}  onClick={handleShow}><MdModeEdit /></Button>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
       <Row>
      <label>username</label> <input style={{margin:"5px",width:"300px"}}  placeholder={username} onChange={(evt)=>{setusername(evt.target.value)}}/>        
       <label>email</label> <input  style={{margin:"5px",width:"300px"}}  placeholder={email} onChange={(evt)=>{setemail(evt.target.value)}}/>        
         <label>Address</label><input  style={{margin:"5px",width:"300px"}}  placeholder={Address} onChange={(evt)=>{setAddress(evt.target.value)}}/>  
       <br/>
